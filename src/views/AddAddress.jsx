...
const AddAddress = () => {
  ...
  const edit_address = async() =>{
    ...
    .then(async response => {
      ...
      console.log('this is response for post call ---->',response.data.result)
  ...
  const add_address = async () => {
    try {
      ...
      // Make the API call
      const response = await axios.post(api_url + customer_add_address, {
        customer_id: global.id,
        address: address.toString(),
        landmark: landmark,
        lat: latitude,
      ...
      if (response.data.status === 1) {
        ...
        handleBackButtonClick(); // Navigate back
      } else {
        console.warn('Error in response:', response.data.message);
        alert(response.data.message); // Show error message
      }
    } catch (error) {
      // Handle any network or API errors
      console.error('Error adding address:', error.message);
      alert('Sorry, something went wrong while adding the address.');
    } finally {
      // Hide the loader in both success and error cases
      setLoading(false);
    }
  };
