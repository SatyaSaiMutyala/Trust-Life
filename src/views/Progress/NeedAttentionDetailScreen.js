...
const CONDITION_CHARTS = {
    Liver: {
        ...
        normalValues: {
            title: 'Normal Values',
            rows: [
                { label: 'ALT (Alanine Aminotransferase / SGPT)', value: '7\u201356 U/L' },
                { label: 'AST (Aspartate Aminotransferase / SGOT)', value: '10\u201340 U/L' },
                { label: 'ALP (Alkaline Phosphatase)', value: '44\u2013147 U/L' },
                { label: 'GGT (Gamma-Glutamyl Transferase)', value: '9\u201348 U/L' },
                { label: 'Total Bilirubin', value: '0.3\u20131.2 mg/dL' },
            ],
            desc: 'Keep track of your results\u2014compare them with the normal values to understand your health',
        },
        charts: [
            {
                label: 'ALT (Alanine Aminotransferase / SGPT)',
                color: '#3B82F6',
                tooltipValue: '85 U/L',
                highlightIndex: 2,
                points: [
            ...
            {
                label: 'AST (Aspartate Aminotransferase / SGOT)',
                color: '#3B82F6',
                tooltipValue: '70 U/L',
                highlightIndex: 2,
                points: [
...
const AreaChart = ({ chartData, chartIndex }) => {
    ...
    const gradientId = `naFill_${chartIndex}`;
...
// ── Main Screen ──────────────────────────────────────────────────────────
const NeedAttentionDetailScreen = () => {
    const navigation = useNavigation();
    const route = useRoute();
    const condition = route.params?.condition || {};

    const conditionName = condition.name || 'Liver';
    const config = CONDITION_CHARTS[conditionName] || CONDITION_CHARTS.Liver;
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.headerTextWrap}>
                        ...
                        <Text style={styles.headerSubtitle}>Need Attention</Text>
                    </View>
...
export default NeedAttentionDetailScreen;
