...
const CONDITION_CHARTS = {
    Pneumonia: {
        description: 'Your condition needs attention. Watch your health and stay in touch with your doctor',
        timelineSubtext: 'View your past readings to identify trends and monitor whether your pneumonia is improving or worsening.',
        normalValues: {
            title: 'Normal Values',
            rows: [
                { label: 'Oxygen Saturation (SpO\u2082)', value: '95–100%' },
                { label: 'C-Reactive Protein (CRP)', value: '< 1 mg/L' },
                { label: 'White Blood Cell (WBC) Count', value: '4,000 – 11,000 /\u03BCL' },
                { label: 'Procalcitonin (PCT)', value: '< 0.1 ng/mL' },
            ],
    ...
    Sepsis: {
        description: 'Your health is okay but needs attention. Keep checking your symptoms and stay in touch with your doctor.',
        timelineSubtext: 'View your past readings to identify trends and monitor whether your sepsis is improving or worsening.',
        normalValues: {
            title: 'Normal Values',
            rows: [
                { label: 'Lactate', value: '0.5–2.2 mmol/L' },
                { label: 'Map', value: '70–100 mmHg' },
                { label: 'C-Reactive Protein (CRP)', value: '< 1 mg/L' },
                { label: 'Procalcitonin (PCT)', value: '< 0.1 ng/mL' },
                { label: 'White Blood Cell (WBC) Count', value: '4,000 – 11,000 /\u03BCL' },
            ],
        ...
        charts: [
            {
                label: 'Lactate',
                color: '#3B82F6',
                tooltipValue: '4 mmol/L',
                highlightIndex: 2,
                points: [
                    { x: 0, y: 0.65 }, { x: 0.12, y: 0.42 }, { x: 0.22, y: 0.18 },
                    { x: 0.33, y: 0.25 }, { x: 0.44, y: 0.20 }, { x: 0.55, y: 0.38 },
                    { x: 0.65, y: 0.30 }, { x: 0.75, y: 0.42 }, { x: 0.85, y: 0.35 },
                    { x: 1, y: 0.28 },
                ],
            },
            {
                label: 'MAP',
                color: '#3B82F6',
                tooltipValue: '65 mmHg',
                highlightIndex: 2,
                points: [
                    { x: 0, y: 0.70 }, { x: 0.12, y: 0.48 }, { x: 0.22, y: 0.22 },
                    { x: 0.33, y: 0.28 }, { x: 0.44, y: 0.18 }, { x: 0.55, y: 0.40 },
                    { x: 0.65, y: 0.32 }, { x: 0.75, y: 0.45 }, { x: 0.85, y: 0.38 },
                    { x: 1, y: 0.30 },
                ],
            },
            {
                label: 'CRP',
                color: '#3B82F6',
                tooltipValue: '200 mg/L',
                highlightIndex: 2,
                points: [
                    { x: 0, y: 0.72 }, { x: 0.12, y: 0.50 }, { x: 0.22, y: 0.20 },
                    { x: 0.33, y: 0.25 }, { x: 0.44, y: 0.15 }, { x: 0.55, y: 0.35 },
                    { x: 0.65, y: 0.28 }, { x: 0.75, y: 0.40 }, { x: 0.85, y: 0.32 },
                    { x: 1, y: 0.25 },
                ],
            },
            {
                label: 'PCT',
                color: '#3B82F6',
                tooltipValue: '6 ng/mL',
                highlightIndex: 2,
                points: [
                    { x: 0, y: 0.68 }, { x: 0.12, y: 0.45 }, { x: 0.22, y: 0.18 },
                    { x: 0.33, y: 0.22 }, { x: 0.44, y: 0.16 }, { x: 0.55, y: 0.36 },
                    { x: 0.65, y: 0.28 }, { x: 0.75, y: 0.42 }, { x: 0.85, y: 0.34 },
                    { x: 1, y: 0.26 },
                ],
            },
            {
                label: 'WBC count',
                color: '#3B82F6',
                tooltipValue: '19,000 /\u03BCL',
                highlightIndex: 2,
                points: [
                    { x: 0, y: 0.74 }, { x: 0.12, y: 0.52 }, { x: 0.22, y: 0.24 },
                    { x: 0.33, y: 0.30 }, { x: 0.44, y: 0.22 }, { x: 0.55, y: 0.42 },
                    { x: 0.65, y: 0.34 }, { x: 0.75, y: 0.48 }, { x: 0.85, y: 0.40 },
                    { x: 1, y: 0.32 },
                ],
            },
        ],
    ...
    Malnutrition: {
        ...
        normalValues: {
            title: 'Normal Values',
            rows: [
                { label: 'Body Mass Index (BMI)', value: '18.5 – 24.9 kg/m\u00B2' },
                { label: 'Serum Albumin', value: '3.5 – 5.0 g/dL' },
                { label: 'Hemoglobin', value: '13.5–17.5 g/dL' },
            ],
            desc: 'Keep track of your results—compare them with the normal values to understand your health',
        },
        charts: [
            {
                ...
                color: '#3B82F6',
                tooltipValue: '16.8 Kg',
                highlightIndex: 2,
                points: [
                    { x: 0, y: 0.60 }, { x: 0.12, y: 0.42 }, { x: 0.22, y: 0.20 },
                    { x: 0.33, y: 0.25 }, { x: 0.44, y: 0.18 }, { x: 0.55, y: 0.32 },
                    { x: 0.65, y: 0.28 }, { x: 0.75, y: 0.38 }, { x: 0.85, y: 0.30 },
                    { x: 1, y: 0.24 },
                ],
            },
            {
...
const AreaChart = ({ chartData, chartIndex }) => {
    ...
    const gradientId = `ltFill_${chartIndex}`;
...
// ── Main Screen ──────────────────────────────────────────────────────────
const LifeThreatsDetailScreen = () => {
    const navigation = useNavigation();
    const route = useRoute();
    const condition = route.params?.condition || {};

    const conditionName = condition.name || 'Pneumonia';
    const config = CONDITION_CHARTS[conditionName] || CONDITION_CHARTS.Pneumonia;
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.headerTextWrap}>
                        ...
                        <Text style={styles.headerSubtitle}>Life threats</Text>
                    </View>
...
export default LifeThreatsDetailScreen;
