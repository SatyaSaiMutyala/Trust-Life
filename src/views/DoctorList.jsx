...
const DoctorList = (props) => {
  ...
  const get_time = async() =>{ 
    console.log(doctor_id,consultation_fee, date)
    await axios({
      method: 'post', 
  ...
  const render_online_doctors = ({ item }) => (
      ...
      >
        ...
        {item.online_status == 1 ?
          ...
          :
          <View style={{ alignItems:'center', width:'60%'}}>
            <View style={{ width:'80%', alignItems:'center', backgroundColor:colors.grey , borderColor:colors.green, borderWidth:1, borderBottomLeftRadius:10, borderBottomRightRadius:10, padding:5 ,}} >
              <Text style={{ fontSize:12, color:colors.theme_fg_three, fontFamily:bold, textAlign:'center', letterSpacing:1 }}>Offline</Text>
            </View>
          </View>
        }
        <View style={{ margin:5 }} />
